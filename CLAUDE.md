...
33. **El histórico de códigos tiene dos lecturas, y la del titular no es «la misma con menos botones».** `DocumentVerificationModel.holders` dice para quién se emitió un certificado — el campo no existía, así que «este certificado es de este usuario» no se podía escribir en ninguna parte. Con él, **el operador** (personal interno) ve todo el histórico, compone resúmenes, exporta a un USB y emite; **el titular** ve *sus* certificados y solo eso. Es M2M y no un FK a un único usuario: empezó siendo un FK y pasó a M2M porque un certificado puede ser de varias personas a la vez —los integrantes de una organización, no solo quien lo tramitó— y elegir uno solo dejaba al resto fuera. Cuatro cosas que no se pueden romper. Una: **el recorte vive en el queryset** (`access.visible_registrations()`, filtrando `document__holders=user`), aplicado **antes** de buscar y antes de agrupar, y el detalle de un código ajeno responde **404** — un 403 confirmaría que ese código existe (invariantes 7 y 20), y contar resultados de una búsqueda ya es media respuesta. Dos: al titular no se le enseñan **las tres piezas con las que se monta un certificado** —el original sin códigos, los símbolos sueltos en PNG y las coordenadas donde se estamparon—, y las tres salen de la misma pregunta (`can_see_internals`) a propósito: separadas, un día se contesta distinto a una por descuido. Y no se esconden, **no se producen**: renderizar un QR para no enseñarlo lo deja a una línea de plantilla de distancia de salir. Tres: quitar a alguien de `holders` (o vaciarlo entero) es la versión M2M de lo que antes hacía `on_delete=SET_NULL` en el FK — de las formas de perder un titular, ninguna **ensancha el acceso**: sin ningún titular no encaja nadie, y quitar a uno de varios no le abre nada a los que quedan ni se lo quita a ellos. Y cuatro: el papel **no depende de `user_type`** sino de ser titular de algo; atarlo al tipo de cuenta dejaría fuera a un tipo nuevo sin que nadie lo notara. Dos consecuencias que ya desconcertaron una vez. **Impersonar enseña lo que ve esa persona**, y eso incluye el recorte: `impersonate` va el último del middleware y sustituye `request.user`, así que impersonar a un titular enseña lo suyo — pero impersonar a alguien **de personal interno enseña todo**, porque es lo que esa cuenta ve por su cuenta. No es un fallo del filtro, y perseguirlo como si lo fuera cuesta una tarde. Y **el enlace tiene que estar donde la vista**: abrir el historial al titular y dejar su entrada dentro del bloque de `is_staff` lo dejaba sin forma de llegar a lo suyo que no fuera saberse la URL. Lo cubre `code_gen/tests/test_holder_history.py`.
...
DocumentVerificationModel  ─M2M─  UserModel (holders)     (para quiénes se emitió, uno o
                                                           varios; decide qué ven en el
                                                           histórico)
